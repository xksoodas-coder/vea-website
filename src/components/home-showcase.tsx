...
export default function HomeShowcase({
  ...
  /* Open on the first category that actually has products: a freshly created
     but still empty category would otherwise greet visitors with the empty
     state instead of the catalogue. */
  const firstFilled =
    categories.find((c) => products.some((p) => p.categoryIds.includes(c.id)))?.id ??
    categories[0]?.id ??
    "";
